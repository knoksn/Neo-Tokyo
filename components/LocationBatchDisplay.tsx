...
import { BatchLocationProfile } from '../services/geminiService';
import { CopyIcon, CheckIcon } from './icons';

interface LocationBatchDisplayProps {
  profiles: BatchLocationProfile[];
}

const LocationCard: React.FC<{ profile: BatchLocationProfile }> = ({ profile }) => {
    ...
    return (
        <div className="bg-slate-900/50 backdrop-blur-sm border border-slate-700/50 rounded-lg p-6 shadow-lg shadow-cyan-500/10 animate-fade-in space-y-4">
            ...
            <div className="border-t border-slate-700/50 pt-4 space-y-3">
                <div>
                    <h3 className="font-bold text-cyan-400 mb-1">Description</h3>
                    <p className="text-gray-300 text-sm">{profile.description}</p>
                </div>
                <div>
                    <h3 className="font-bold text-cyan-400 mb-1">Key Feature / NPC</h3>
                    <p className="text-gray-300 text-sm">{profile.key_feature_or_npc}</p>
                </div>
            </div>
...
const LocationBatchDisplay: React.FC<LocationBatchDisplayProps> = ({ profiles }) => {
  ...
  return (
    <div className="mt-8 space-y-8">
        <header className="text-center">
            <h1 className="text-3xl font-bold text-cyan-300">Generated Location Profiles</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {profiles.map(profile => (
                <LocationCard key={profile.name} profile={profile} />
            ))}
        </div>
    </div>
  );
...
export default LocationBatchDisplay;