...
import StoryboardDisplay from './DlcDisplay';
import LoadingSpinner from './LoadingSpinner';
import { SparklesIcon, QuestIcon, MoodIcon } from './icons';
...
const StoryboardGenerator: React.FC<StoryboardGeneratorProps> = ({ generatedStoryboard, setGeneratedStoryboard }) => {
  const [chapter, setChapter] = useState('Chapter 2: Hive Mind');
  const [theme, setTheme] = useState('Brain-dance den infiltration');
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);

  const handleGenerate = useCallback(async () => {
    if (!chapter || !theme) {
      setError('Chapter/Scene and Theme must be filled out.');
      return;
    }
    setIsLoading(true);
    setError(null);
    ...
    try {
      const sceneDescription = `${chapter}: ${theme}`;
  ...
  }, [chapter, theme, setGeneratedStoryboard]);
      ...
      <div className="bg-slate-900/50 backdrop-blur-sm border border-slate-700/50 rounded-lg p-6 shadow-lg shadow-cyan-500/5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           <InputGroup
            ...
            placeholder="e.g., Chapter 2: Hive Mind"
            icon={<QuestIcon />}
          />
           <InputGroup
            label="Theme"
            ...
            placeholder="e.g., Brain-dance den infiltration"
            icon={<MoodIcon />}
          />
        </div>
